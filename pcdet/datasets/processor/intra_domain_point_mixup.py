...
def intra_domain_point_mixup(data_dict_1, data_dict_2, alpha=None):
    ...
    new_data_dict["points"] = np.concatenate(
        (
            data_dict_1["points"][: int(data_dict_1["points"].shape[0] * lam)],
            data_dict_2["points"][: int(data_dict_2["points"].shape[0] * (1 - lam))],
        ),
        axis=0,
    )
    new_data_dict["gt_boxes"] = np.concatenate(
        (data_dict_1["gt_boxes"], data_dict_2["gt_boxes"]), axis=0
    )
...
def intra_domain_point_mixup_cd(data_dict_1, data_dict_2, alpha=None):
    ...
    try:
        ...
        iou = iou3d_nms_utils.boxes_bev_iou_cpu(
            data_dict_1["gt_boxes"][:, 0:7], data_dict_2["gt_boxes"][:, 0:7]
        )
    ...
    new_data_dict["gt_boxes"] = np.concatenate(
        (data_dict_1["gt_boxes"], valid_boxes), axis=0
    )
