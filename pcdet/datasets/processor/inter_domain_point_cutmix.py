...
def inter_domain_point_cutmix(data_source, data_target, pc_range):
    ...
    assert (
        len(cutmixed_data["points"]) == 0
    ), "new generated cutmixed_data should contain 0 point before inter_domain point_cutmix!"
    cutmixed_data["gt_boxes"] = []
    assert (
        len(cutmixed_data["gt_boxes"]) == 0
    ), "new generated cutmixed_data should contain 0 gt_box before inter_domain point_cutmix!"
    ...
    while True:
        ...
        sample_center = data_source["points"][
            np.random.choice(len(data_source["points"])), 0:3
        ]
        # print(sample_center)
        ...
        upper_idx_source = (
            np.sum((data_source["points"][:, :2] < max_xy).astype(np.int32), 1) == 2
        )
        lower_idx_source = (
            np.sum((data_source["points"][:, :2] > min_xy).astype(np.int32), 1) == 2
        )

        upper_idx_target = (
            np.sum((data_target["points"][:, :2] < max_xy).astype(np.int32), 1) == 2
        )
        lower_idx_target = (
            np.sum((data_target["points"][:, :2] > min_xy).astype(np.int32), 1) == 2
        )
    ...
    cutmixed_data["points"].extend(
        data_target["points"][inside_region_point_idx_target, :]
    )
    cutmixed_data["points"].extend(
        data_source["points"][outside_region_point_idx_source, :]
    )

    assert (
        len(cutmixed_data["points"]) != 0
    ), "new generated cutmixed_data should contain more than 0 point after inter_domain point_cutmix!"
    ...
    inside_region_gt_boxes_mask_source = mask_boxes_outside_range_numpy(
        data_source["gt_boxes"], region_range, min_num_corners=1
    )
    inside_region_gt_boxes_mask_target = mask_boxes_outside_range_numpy(
        data_target["gt_boxes"], region_range, min_num_corners=1
    )

    inside_region_gt_boxes_source = data_source["gt_boxes"][
        inside_region_gt_boxes_mask_source
    ]
    inside_region_gt_boxes_target = data_target["gt_boxes"][
        inside_region_gt_boxes_mask_target
    ]
    outside_region_gt_boxes_source = data_source["gt_boxes"][
        ~inside_region_gt_boxes_mask_source
    ]
    outside_region_gt_boxes_target = data_target["gt_boxes"][
        ~inside_region_gt_boxes_mask_target
    ]
