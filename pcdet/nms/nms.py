...
def nms(
    ...
    selected_boxes = []
    for query_idx, bbox in enumerate(boxes_probability_sorted):
        if bbox[0] > 0:
            selected_boxes.append(bbox)
            for value_idx, other_box in enumerate(boxes_probability_sorted):
                if (
                    query_idx < value_idx
                    and custom_nms.iou(bbox[1:], other_box[1:]) > iou_thres
                ):
                    other_box[0] = 0
...
#         related_idxes = np.asarray(
#             [i for i in related_idxes if inst_types[i] == inst_types[index]]
#         )
...
#             yaw_vote = np.where(
#                 np.abs(yaws[order_vote] - median_yaw) % (2 * np.pi) < threshold_yaw
#             )[0]
...
#             avg_bbox_array = (
#                 np.sum(scores[order_vote][:, np.newaxis] * det_arrays, axis=0)
#                 / vote_score_sum
#             )
