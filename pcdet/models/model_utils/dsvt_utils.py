...
@torch.no_grad()
def get_window_coors(
    coors, sparse_shape, window_shape, do_shift, shift_list=None, return_win_coors=False
):
    # coors : voxel 좌표
    # sparse_shape : from config, 전체 3D space 크기 , [468, 468, 32]
    # window_shape : from config, 윈도우 크기 , [[[12, 12, 32], [12, 12, 8], [12, 12, 2], [12, 12, 1]]]
    if len(window_shape) == 2:  # window가 2D 인지 3D 인지 확인
    ...
    # 각 축에 대해 최대 윈도우 개수를 계산
    max_num_win_x = int(
        np.ceil((sparse_shape_x / win_shape_x)) + 1
    )  # plus one here to meet the needs of shift.
    max_num_win_y = int(
        np.ceil((sparse_shape_y / win_shape_y)) + 1
    )  # plus one here to meet the needs of shift.
    max_num_win_z = int(
        np.ceil((sparse_shape_z / win_shape_z)) + 1
    )  # plus one here to meet the needs of shift.
    max_num_win_per_sample = max_num_win_x * max_num_win_y * max_num_win_z
    # shifts_list : from cfg,shifts_list: [[[0, 0, 0], [6, 6, 0]], [[0, 0, 0], [6, 6, 0]], [[0, 0, 0], [6, 6, 0]], [[0, 0, 0], [6, 6, 0]]]
    if do_shift:
...
def get_pooling_index(coors, sparse_shape, window_shape):
    ...
    index_in_win = (
        coors_in_win_x * win_shape_y * win_shape_z
