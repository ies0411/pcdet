...
class CenterHead(nn.Module):
    def __init__(
        ...
        for cur_class_names in self.model_cfg.CLASS_NAMES_EACH_HEAD:
            ...
            cur_class_id_mapping = torch.from_numpy(
                np.array(
                    [
                        self.class_names.index(x)
                        for x in cur_class_names
                        if x in class_names
                    ]
                )
            ).cuda()
        ...
        for idx, cur_class_names in enumerate(self.class_names_each_head):
            ...
            cur_head_dict["hm"] = dict(
                out_channels=len(cur_class_names), num_conv=self.model_cfg.NUM_HM_CONV
            )
    ...
    def assign_target_of_single_head(
        ...
        heatmap = gt_boxes.new_zeros(
            num_classes, feature_map_size[1], feature_map_size[0]
        )
        ...
        coord_x = (
            (x - self.point_cloud_range[0]) / self.voxel_size[0] / feature_map_stride
        )
        coord_y = (
            (y - self.point_cloud_range[1]) / self.voxel_size[1] / feature_map_stride
        )
        coord_x = torch.clamp(
            coord_x, min=0, max=feature_map_size[0] - 0.5
        ...
        for k in range(min(num_max_objs, gt_boxes.shape[0])):
            ...
            if not (
                0 <= center_int[k][0] <= feature_map_size[0]
            ...
            if "iou" in pred_dict or self.model_cfg.get("IOU_REG_LOSS", False):
                ...
                if self.model_cfg.get("IOU_REG_LOSS", False):
                    ...
                    if target_dicts["masks"][idx].sum().item() != 0:
                        ...
                        tb_dict["iou_reg_loss_head_%d" % idx] = (
                            batch_box_preds_for_iou * 0.0
                        ).sum()
    ...
    def generate_predicted_boxes(self, batch_size, pred_dicts):
        ...
        post_center_limit_range = (
            torch.tensor(post_process_cfg.POST_CENTER_LIMIT_RANGE).cuda().float()
        )
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            batch_vel = (
                pred_dict["vel"] if "vel" in self.separate_head_cfg.HEAD_ORDER else None
            )
            ...
            for k, final_dict in enumerate(final_pred_dicts):
                ...
                if (
                    post_process_cfg.get("USE_IOU_TO_RECTIFY_SCORE", False)
                    and "pred_iou" in final_dict
                ):
                    ...
                    final_dict["pred_scores"] = torch.pow(
                        ...
                        1 - IOU_RECTIFIER[final_dict["pred_labels"]],
        ...
        for k in range(batch_size):
            ...
            ret_dict[k]["pred_labels"] = (
                torch.cat(ret_dict[k]["pred_labels"], dim=0) + 1
            )
