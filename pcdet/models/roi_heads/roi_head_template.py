...
class RoIHeadTemplate(nn.Module):
    ...
    def get_box_cls_layer_loss(self, forward_ret_dict):
        ...
        elif loss_cfgs.CLS_LOSS == "BinaryCrossEntropyWithLogits":
            rcnn_cls_flat = rcnn_cls.view(-1)
            # print("check")
            batch_loss_cls = F.binary_cross_entropy_with_logits(
                rcnn_cls_flat, rcnn_cls_labels
            )
            # print(batch_loss_cls)
            cls_valid_mask = (rcnn_cls_labels >= 0).float()
            rcnn_loss_cls = (batch_loss_cls * cls_valid_mask).sum() / torch.clamp(
                cls_valid_mask.sum(), min=1.0
            )
