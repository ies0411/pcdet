...
class Stage_ReductionAtt_Block(nn.Module):
    ...
    def forward(self, x, key_padding_mask):
        ...
        # Convert key_padding_mask to boolean type if it's not
...
class DSVTInputLayer(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        self.window_shape = [
            [
                ...
                [
                    self.window_shape[s_id][coord_id] * self.hybrid_factor[coord_id]
            ...
            for j in range(self.set_info[i][1]):
                ...
                for s in range(self.num_shifts[i]):
                    block_posembed_layers.append(
                        PositionEmbeddingLearned(input_dim, self.d_model[i])
                    )
        ...
        for stage_id in range(self.stage_num):
            ...
            for block_id in range(self.set_info[stage_id][1]):
                for shift_id in range(self.num_shifts[stage_id]):
                    voxel_info[
                        f"pos_embed_stage{stage_id}_block{block_id}_shift{shift_id}"
                    ] = self.get_pos_embed(
                        voxel_info[f"coors_in_win_stage{stage_id}_shift{shift_id}"],
                        stage_id,
                        block_id,
                        shift_id,
                    )
    ...
    def get_set_single_shift(
        ...
        max_voxel = (
            self.window_shape[stage_id][shift_id][0]
        ...
        global_voxel_inds_sorty = (
            contiguous_win_inds * max_voxel
            + coors_in_win[:, 1]
            * self.window_shape[stage_id][shift_id][0]
            * self.window_shape[stage_id][shift_id][2]
        ...
        )  # get y-axis ordered inner window indexs of each voxel
        voxel_inds_in_batch_sorty = (
            inner_voxel_inds_sorty + max_voxel * contiguous_win_inds
        )
        ...
        inner_voxel_inds_sortx.scatter_(
            dim=0, index=order2, src=inner_voxel_inds[order1]
        )  # get x-axis ordered inner window indexs of each voxel
        voxel_inds_in_batch_sortx = (
            inner_voxel_inds_sortx + max_voxel * contiguous_win_inds
        )
