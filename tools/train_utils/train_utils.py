...
def train_one_epoch(
    ...
    scaler = torch.cuda.amp.GradScaler(
        enabled=use_amp, init_scale=optim_cfg.get("LOSS_SCALE_FP16", 2.0**16)
    )
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        if rank == 0:
            ...
            if use_logger_to_record:
                if (
                    accumulated_iter % logger_iter_interval == 0
                    or cur_it == start_it
                    or cur_it + 1 == total_it_each_epoch
                ):
                    trained_time_past_all = tbar.format_dict["elapsed"]
                    second_each_iter = pbar.format_dict["elapsed"] / max(
                        cur_it - start_it + 1, 1.0
                    )

                    trained_time_each_epoch = pbar.format_dict["elapsed"]
                    remaining_second_each_epoch = second_each_iter * (
                        total_it_each_epoch - cur_it
                    )
                    ...
                    logger.info(
                        ...
                        "Batch time: {batch_time.val:.2f}({batch_time.avg:.2f})".format(
                            cur_epoch + 1,
                            total_epochs,
                            100.0 * (cur_epoch + 1) / total_epochs,
                            cur_it,
                            total_it_each_epoch,
                            100.0 * cur_it / total_it_each_epoch,
                            loss=losses_m,
                            lr=cur_lr,
                    ...
                    if (
                        show_gpu_stat
                        and accumulated_iter % (3 * logger_iter_interval) == 0
                    ):
...
def disable_augmentation_hook(
    hook_config, dataloader, total_epochs, cur_epoch, cfg, flag, logger
):
    """
    ...
    if hook_config is not None:
        ...
        if DisableAugmentationHook is not None:
            ...
            if (total_epochs - num_last_epochs) <= cur_epoch and not flag:
                ...
                dataloader._dataset.data_augmentor.disable_augmentation(
                    dataset_cfg.DATA_AUGMENTOR
                )
                flag = True
    return flag
